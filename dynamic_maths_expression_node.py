...
class _DynamicMathsExpression_Operator_common(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        return (
            ...
            and (hasattr(space.node_tree, "nodes"))
        )
    ...
    def invoke(self, context, event):
        ...
        if self.mode == "EDIT":
            ...
            for node in tree.nodes:
                if (
                    hasattr(node, "bl_name")
                    and node.bl_name == "DynamicMathsExpressionNode"
                ):
                    expressionNode = node
            ...
            if self.mode in ("EDITWITHIN", "UPDATE", "REVERT"):
                ...
                if expressionNode == None:
                    self.report(
                        {"ERROR"}, "Selected node is not an Expression Group Node"
                    )
                    return {"CANCELLED"}
    ...
    def execute(self, context):
        if self.mode == "CREATE":
            ...
            bpy.ops.ed.undo_push(
                message="Revert a Maths Expression node back to its previous state"
            )
        elif self.mode == "UPDATE":
            bpy.ops.ed.undo_push(
                message="Update a Maths Expression node from its text block"
            )
        else:
    ...
    def __nodetree_setup__(self):
            ...
            while True:
                if operations[0] == ",":
                    ...
                    if found < 0:
                        ...
                        socket = self.node_tree.nodes["Group Output"].inputs[
                            len(self.node_tree.nodes["Group Output"].inputs) - 2
                        ]  # TODO: Fix this fudge
                    else:
                        socket = self.node_tree.nodes["Group Output"].inputs[found]

                    self.build_nodes(
                        expr, socket, self.node_tree.nodes["Group Output"].location, 0
                    )
        ...
        groupOutputNode.location[0] = (
            exprNode.location[0] + exprNode.width - groupOutputNode.width
        )  # Position output at right-edge of exprNode
        # groupOutputNode.location[1] = exprNode.location[1] - 350                                                # Position output just below exprNode

        groupInputNode.location[0] = exprNode.location[
            0
        ]  # Position input at left-edge of exprNode
        groupInputNode.location[1] = groupOutputNode.location[
            1
        ]  # Position input just below exprNode
    ...
    def processColorRampExpression(self, node, expr):
        ...
        for exprTerm in Expression.convertCommaExpressionToList(expr):
            ...
            if exprTerm[0] == "value":
                ...
                if pairCount == 1:
                    raise Exception(
                        "COLORRAMP got 'combine' in place of a position - position must"
                        " be a plain value"
                    )
                else:
                    col = [0.0, 0.0, 0.0]
                    colIdx = 0
                    for combineTerm in Expression.convertCommaExpressionToList(
                        exprTerm[1]
                    ) + Expression.convertCommaExpressionToList(exprTerm[2]):
                        if combineTerm[0] == "value":
                            ...
                            raise Exception(
                                "COLORRAMP combine got an unexpected expression ('"
                                + combineTerm[0]
                                + "')"
                            )
                ...
                raise Exception(
                    "COLORRAMP got an unexpected expression ('" + exprTerm[0] + "')"
                )

        if pairCount != 0:
            raise Exception(
                "COLORRAMP received the wrong number of arguments - must pair each"
                " 'value' with a 'color'"
            )
    ...
    def build_nodes(
        self, nested_operations, to_output, output_location, depth, lastoperation=None
    ):
        depth += 1
        ...
        if operation == "variable":
            ...
            (variableName, variableDefault) = Expression.extractVariableDefault(
                variableName
            )
            ...
            (variableName, variableSubscript) = Expression.extractVariableSubscript(
                variableName
            )
            ...
            if variableSubscript != None:
                ...
                if separateNode == None:
                    ...
                    if (
                        variableName
                        in self.node_tree.nodes["Group Input"].outputs.keys()
                    ):
                        self.node_tree.links.new(
                            self.node_tree.nodes["Group Input"].outputs[variableName],
                            separateNode.inputs[0],
                        )
                    else:
                        # Add a new socket
                        self.node_tree.inputs.new("NodeSocketVector", variableName)
                        self.node_tree.links.new(
                            self.node_tree.nodes["Group Input"].outputs[-2],
                            separateNode.inputs[0],
                        )  # Add link to group input we just added (-1 is 'blank' socket, -2 is last 'real' socket)
                ...
                if variableName in self.node_tree.nodes["Group Input"].outputs.keys():
                    ...
                    self.node_tree.links.new(
                        self.node_tree.nodes["Group Input"].outputs[-2], to_output
                    )  # Add link to group input we just added (-1 is 'blank' socket, -2 is last 'real' socket)
            ...
            # Repeat for sub-nodes
            self.build_nodes(
                nested_operations[1], newnode.inputs[0], newlocation, depth
            )
            if nodetype.startswith("ShaderNodeTex"):
                if len(nested_operations) > 2:
                    ...
                    newlocation[1] += 1 / depth
                    self.build_nodes(
                        nextNestedOp, newnode.inputs[socketNo], newlocation, depth
                    )
            else:
                if (
                    len(nested_operations) > 2 and operation != "colorramp"
                ):  # (for colorramp we'll have already processed sub-expressions as configuration for the node)
                    newlocation[1] += 1 / depth
                    self.build_nodes(
                        nested_operations[2],
                        newnode.inputs[1],
            ...
            if defaultValue != None:
                if input.type == "VECTOR":
                    input.default_value = [
                        ...
                        float(defaultValue),
                        float(defaultValue),
                    ]  # TODO: Perhaps support defaulting to an actual vector rather than a float converted to a vector. Also, possibly support other types....?
                else:
                    input.default_value = float(
                        defaultValue
                    )  # TODO: Detect socket type and handle different types (int, string?, Vector, etc.)
        ...
        for groupinput in self.node_tree.nodes["Group Input"].outputs:
            ...
            if nodeid != None:
                ...
                for groupinputlink in groupinput.links:
                    self.node_tree.links.new(
                        node.outputs[socket], groupinputlink.to_socket
                    )
    ...
    def resequence_inputs(self):
        ...
        for socketNo in range(1, len(inputs)):
            ...
            while compareSocketNo >= 0:
                ...
                if (
                    self.resequence_inputs_comparator(thisSocketName, compareSocketName)
                    < 0
                ):
    ...
    def execute_main(self, context):
        ...
        if (
            ...
            if self.mode == "CREATE":
                ...
                groupnode.node_tree = bpy.data.node_groups.new(
                    "DynamicMathsExpressionGroup", "ShaderNodeTree"
                )
                groupnode.node_tree.nodes.new("NodeGroupInput")
                groupnode.node_tree.nodes.new("NodeGroupOutput")

                # Position in centre
                groupnode.location = context.region.view2d.region_to_view(
                    context.region.width / 2 - groupnode.width / 2,
                    context.region.height / 2 + groupnode.height / 2,
                )
    ...
    def draw(self, context):
        ...
        documentationFile = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "documentation.txt"
        )
        if os.path.isfile(
            documentationFile
        ):  # and not "Node Expressions Documentation" in bpy.data.texts.keys():
            split.operator(
                operator="node.node_dynamic_maths_expression_showdocumentation",
                text="View Documentation",
            )
        upgradeFile = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "upgrade.txt"
        )
        if os.path.isfile(upgradeFile):
            split.operator(
                operator="node.node_dynamic_maths_expression_showupgrade",
                text="How to upgrade",
            )
...
class DynamicMathsExpression_Operator(_DynamicMathsExpression_Operator_common):
    ...
    expressionText: bpy.props.StringProperty(
        description="Enter the expression", name="Expression"
    )
    resequenceInputs: bpy.props.BoolProperty(
        name="Auto-sequence Inputs",
        description="Automatically resequence input sockets",
    )
...
class DynamicMathsExpressionEdit_Operator(_DynamicMathsExpression_Operator_common):
    ...
    createNew: bpy.props.BoolProperty(
        ...
        description=(
            "Create a new group rather than updating the existing one (which would"
            " affect all group nodes using this group)"
        ),
    )
    pruneSockets: bpy.props.BoolProperty(
        name="Prune Inputs/Outputs", description="Automatically remove unused sockets"
    )
    resequenceInputs: bpy.props.BoolProperty(
        name="Auto-sequence Inputs",
        description="Automatically resequence input sockets",
...
class DynamicMathsExpressionEditWithin_Operator(
    _DynamicMathsExpression_Operator_common
):
...
class DynamicMathsExpressionShowDocumentation_Operator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        otherarea = (
            context.area
        )  # Instead of using the new one, use original (the right-hand split)
        newarea = context.screen.areas[
            -1
        ]  # Assumed it's the last one in the stack (which it should be)
...
class DynamicMathsExpressionNode(bpy.types.NodeCustomGroup):
    ...
    storedValidExpression: bpy.props.StringProperty(
        description="", name="Saved Expression"
    )
    ...
    def draw_buttons(self, context, layout):
        ...
        # row=layout.row()
        row.operator(
            "node.node_dynamic_maths_expression_editwithin", text="Edit"
        ).node_tree_name = self.id_data.name
        # Get group this node resides in

        layout.alert = self.enableRevert
        if self.enableRevert and self.storedValidExpression != "":
            row = layout.row()
            row.operator(
                "node.node_dynamic_maths_expression_revert",
                text="Node is invalid! - Click to Revert",
            ).node_tree_name = self.id_data.name
            # Get group this node resides in
